...
class _MyWidgetState extends State<AttendanceWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      ownLogs = logs!
          .where((log) => log.guID == employeeProfile[4])
          .toList();
  ...
  Future<void> _selectDate(BuildContext context) async {
    ...
    if (picked != null && picked != startDate) {
      setState(() {
        startDate = picked;
        if(isEmployee){
          ownLogs = sortList(getDateRange(employeeProfile[0], startDate, endDate, logs!));
        }
        else{
          ownLogs = sortList(getDateRange(dropdownvalue, startDate, endDate, logs!));
        }
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return LoaderView(child: Container(
        width: 100.w,
        child: Column(children: [ Visibility(visible: isEmployee == false, child: Container(child: DropdownButton(
          value: dropdownvalue,
          items: employeeList != null ? employeeList?.map((e) {
                ...
                setState(() {
                  ...
                  if(isDateSelected == false){
                    ...
                    ownLogs = sortList(getDateRange(dropdownvalue, startDate, endDate, ownLogs!));
                  }
          ...
          Container(
      width: 100.w,
        child: Padding(padding: EdgeInsets.only(left: 6.w), child:DataTable(
          dataRowHeight: 5.h,
          columns: [
